...
# import pygenometracks.tracks
import GenomeTrack as GenomeTrack
# from .GenomeTrack import GenomeTrack
DEFAULT_BIGWIG_COLOR = '#33a02c'

import HTSeq

class DevBigWigTrack( GenomeTrack.GenomeTrack):
    SUPPORTED_ENDINGS = ['.bw', '.bigwig']
    TRACK_TYPE = 'bigwig'
    OPTIONS_TXT = GenomeTrack.GenomeTrack.OPTIONS_TXT + """
color = #666666
...
file_type = {}
    ...
    def plot(self, ax, chrom_region, start_region, end_region):
        ...
        def get__chrom_region(self,chrom_region):
        ...
        num_bins = get__num_bins( self)
        chrom_region = get__chrom_region( self, chrom_region)
        scores_per_bin = get__scores_per_bin(self)
        window_iv = HTSeq.GenomicInterval(chrom_region,start_region,end_region)
        self._plot(ax, window_iv, scores_per_bin, )
        
    def _plot(self, ax, iv, scores_per_bin, ):
        x_values = np.linspace( iv.start, iv.end, len(scores_per_bin))
